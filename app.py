...
app = Flask(__name__)
app.config["SECRET_KEY"] = os.environ.get("SECRET_KEY", "sar-brooms-change-this-secret-key-in-production")

# Use Render's PostgreSQL (DATABASE_URL) when available; otherwise fall back
# to a local SQLite file for running on your own computer.
...
if __name__ == "__main__":
    ...
    # When running under a production server (gunicorn, etc. — e.g. on Render),
    # __main__ never executes, so we initialize the database here instead.
    init_db_and_seed()
